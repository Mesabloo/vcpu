...
use crate::component::decoder::Decoder4X16;
use crate::component::memory::register::Register;
use crate::units::bit::ON;
use cell::Cell;
...
impl RAM {
    pub fn new(bus1: Bus, bus2: Bus, sa: Wire, s: Wire, e: Wire) -> Self {
        ...
        let always_on = Wire::default();
        always_on.set(ON);
        let r = Register::new(bus1, sa.clone(), always_on, Bus::from(reg_to_dec_s.clone()));
        ...
        let dh = Decoder4X16::new(reg_to_dh, dh_to_bus.clone());
        let dv = Decoder4X16::new(reg_to_dv, dv_to_bus.clone());
